...
<a href="https://bartoszmilewski.files.wordpress.com/2015/01/functormaybe.jpg"><img class="aligncenter size-medium wp-image-3950" src="https://bartoszmilewski.files.wordpress.com/2015/01/functormaybe.jpg?w=300&#038;h=219" alt="FunctorMaybe" width="300" height="219" /></a> We often say that `fmap` *lifts* a function. The lifted function acts on `Maybe` values. As usual, because of currying, this signature may be interpreted in two ways: as a function of one argument &#8212; which itself is a function `(a->b)` &#8212; returning a function `(Maybe a -> Maybe b)`; or as a function of two arguments returning `Maybe b`:
...
```
template<<span class="red">template<class> F</span>, class A, class B>
...
```
template<class A, class B>
optional<B> fmap<<span class="red">optional</span>>(std::function<B(A)> f, optional<A> opt)
```
...
<ol>
<li>Can we turn the `Maybe` type constructor into a functor by defining:
```
fmap _ _ = Nothing
```

which ignores both of its arguments? (Hint: Check the functor laws.)</li>
...
<li>Prove the functor laws for the list functor. Assume that the laws are true for the tail part of the list you're applying it to (in other words, use *induction*).</li>
</ol>
## Acknowledgments
